...
**Phase 1 Success Criteria** — what must be TRUE when this phase is done:
1. {{Observable user behavior or system behavior — e.g., "User can create an account with email/password"}}
2. {{Observable behavior — e.g., "Invalid credentials show error message, don't crash"}}
...
**Phase 2 Success Criteria** — what must be TRUE when this phase is done:
1. {{Observable behavior — not "API works" but "GET /users returns paginated list with 20 items per page"}}
...
**Phase 3 Success Criteria** — what must be TRUE when this phase is done:
1. {{Observable behavior — verifiable by a human using the application}}
...
- [ ] Every phase has observable success criteria (not "tests pass" but what the user/system can do)
...
**Requires:** T-01 (schema), T-02 (route stubs)
- [ ] T-04: Implement CRUD endpoints
- [ ] T-05: Build form components

### Wave 3 (depends on: Wave 2)
**Requires:** T-04 (endpoints), T-05 (components)
- [ ] T-06: Integration tests
- [ ] T-07: Documentation
```

Each wave declares which specific tasks it depends on. This prevents agents from reordering waves and makes the dependency chain explicit.
